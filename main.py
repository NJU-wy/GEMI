...
class MultiViewDataset(Dataset):
    def __init__(self, indices, raw_slices, phy_slices, ae_slices, macro_slices, label_slices, stats_phy, stats_ae):
        ...
        self.macro_slices, self.label_slices, self.stats_phy, self.stats_ae = macro_slices, label_slices, stats_phy, stats_ae
...
def main():
    ...
    for lbl in np.unique(global_labels):
        ...
        n = len(cells)
        if n == 1:
            train_cells_idx.extend(cells)
        elif n == 2:
            train_cells_idx.extend(cells[:1]); test_cells_idx.extend(cells[1:])
        else:
            p1 = max(1, int(n * 0.7));
            p2 = max(p1 + 1, int(n * 0.85)) if n >= 3 else p1
            if p2 >= n: p2 = n - 1
    ...
    del raw_list
    for_ae_slices = np.array(for_ae_list, dtype=np.float32);
    del for_ae_list
    phy_slices = np.array(phy_list, dtype=np.float32);
    del phy_list
    slice_labels = np.array(label_list, dtype=int);
    del label_list
    slice_cell_indices = np.array(cell_idx_list, dtype=int);
    del cell_idx_list
    ...
    train_macro_mean, train_macro_std = np.mean(macro_slices[idx_train], axis=0), np.std(macro_slices[idx_train],
                                                                                         axis=0) + 1e-6
    macro_slices = (macro_slices - train_macro_mean) / train_macro_std

    print("\n[Phase 3] 训练去噪自编码器 (仅 Train Normal)...")
    ...
    artifacts = {
        ...
        "stats_phy": stats_phy, "stats_ae": stats_ae, "train_cells_idx": train_cells_idx,
        "test_cells_idx": test_cells_idx, "cell_names": cell_names, "win": win
    }
    try:
        ...
        print(f"  ⚠️ [IO Error] 凭证保存失败: {e}")

    print("  -> 🧹 清扫阶段：释放缓存与显存碎片...")
    del ae, ae_loader, inference_loader, train_normal_mask, tune_idx, V, V_for_ae, V_phy_all
    gc.collect();
    torch.cuda.empty_cache()

    print("\n[Phase 4] 启动 Advanced Hybrid SOTA 分类引擎...")
    class_counts = np.zeros(3)
    unique_classes, counts = np.unique(slice_labels[idx_train], return_counts=True)
    ...
    for ep in range(100):
        model.train();
        train_loss = 0
        pbar_train = tqdm(train_loader, desc=f"Epoch {ep + 1:3d}/100 [Train]", leave=False)
        for bx, bmacro, by in pbar_train:
        ...
        pbar_val = tqdm(val_loader, desc=f"Epoch {ep + 1:3d}/100 [Val  ]", leave=False)
        with torch.no_grad():
            for bx, bmacro, by in pbar_val:
